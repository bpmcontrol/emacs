...
;; some search engine (imap, namazu, gmane and others -- see
...
;; data from the search engine can be put in the buffer stored in the
;; engine's tmp-buffer slot.  This function should return the list of
;; articles as a vector, as described above.  Then, you need to
;; register this backend in `nnir-engines'.  Then, users can choose
;; the backend by setting `nnir-search-engine' as a server variable.
...
(defcustom nnir-notmuch-raw-queries-p nil
...
(defcustom nnir-imap-raw-queries-p nil
...
(cl-defmethod nnir-search-indexed-search-command ((engine gnus-search-hyrex)
						  (qstring string))
...
(cl-defmethod nnir-search-indexed-massage-output ((engine gnus-search-hyrex)
						  server &optional groups)
  (let ((groupspec (when groups
		     (regexp-opt
		      (mapcar
...
(cl-defmethod nnir-run-search ((engine gnus-search-gmane)
  ...
  (let* ((case-fold-search t)
	 ...
	 (search (format "%s %s"
			 (if (listp query)
			     (nnir-search-transform-top-level query)
			   query)
			 groupspec))
	 (gnus-inhibit-demon t)
	 artlist)
